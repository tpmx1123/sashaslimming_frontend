import React from 'react'
import { motion } from 'framer-motion'
import instaImage from '../../assets/insta.png'
import facebookImage from '../../assets/facebook.png'

const Footer = () => {
  const containerVariants = {
    hidden: { opacity: 0 },
    visible: {
      opacity: 1,
      transition: {
        staggerChildren: 0.15,
        delayChildren: 0.1,
      },
    },
  }

  const itemVariants = {
    hidden: { opacity: 0, y: 20 },
    visible: {
      opacity: 1,
      y: 0,
      transition: {
        duration: 0.5,
        ease: "easeOut",
      },
    },
  }
  const quickLinks = [
    "ABOUT US",
    "CONCERNS",
    "BEFORE & AFTER",
    "MEDIA",
    "CELEBRITIES @ SASHA",
    "CONTACT US",
    "SHOP HOMECARE"
  ]

  return (
    <motion.footer 
      className="w-full bg-[#392D44]"
      initial="hidden"
      whileInView="visible"
      viewport={{ once: true, amount: 0.2 }}
      variants={containerVariants}
    >
      {/* Main Footer Content */}
      <div className="max-w-5xl mx-auto px-36 sm:px-6 lg:px-3 py-8 h-full">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
          {/* Facebook Section */}
          <motion.div variants={itemVariants}>
            <h3 className="text-[#D4AF37] font-bold text-lg mb-6 uppercase">
              FACEBOOK
            </h3>
            <motion.div 
              className="max-w-[280px] mx-auto lg:mx-0"
              whileHover={{ scale: 1.05 }}
              transition={{ duration: 0.2 }}
            >
              <img 
                src={facebookImage} 
                alt="Facebook" 
                className="w-full h-auto rounded-lg"
              />
            </motion.div>
          </motion.div>

          {/* Instagram Section */}
          <motion.div variants={itemVariants}>
            <h3 className="text-[#D4AF37] font-bold text-lg mb-6 uppercase">
              INSTAGRAM
            </h3>
            <motion.div 
              className="max-w-[280px] mx-auto lg:mx-0"
              whileHover={{ scale: 1.05 }}
              transition={{ duration: 0.2 }}
            >
              <img 
                src={instaImage} 
                alt="Instagram" 
                className="w-full h-auto rounded-lg"
              />
            </motion.div>
          </motion.div>

          {/* Quick Links Section */}
          <motion.div variants={itemVariants}>
            <h3 className="text-[#D4AF37] font-bold text-lg mb-6 uppercase">
              QUICK LINKS
            </h3>
            <motion.div 
              className="space-y-3"
              variants={containerVariants}
            >
              {quickLinks.map((link, index) => (
                <motion.a
                  key={index}
                  href="#"
                  className="inline-block bg-[#483956] text-white uppercase font-medium px-4 py-2 rounded hover:bg-gray-600 transition-colors text-sm"
                  variants={itemVariants}
                  whileHover={{ scale: 1.05, x: 5 }}
                  transition={{ duration: 0.2 }}
                >
                  {link}
                </motion.a>
              ))}
            </motion.div>
          </motion.div>

          {/* Supports Section */}
          <motion.div variants={itemVariants}>
            <h3 className="text-[#D4AF37] font-bold text-lg mb-6 uppercase">
              SUPPORTS
            </h3>
            <div className="space-y-4">
              {/* Phone Numbers */}
              <div className="flex items-center gap-3">
                <svg className="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-1.548.773a11.037 11.037 0 006.105 6.105l.774-1.548a1 1 0 011.059-.54l4.435.74a1 1 0 01.836.986V17a1 1 0 01-1 1h-2C7.82 18 2 12.18 2 5V3z" />
                </svg>
                <a href="tel:+919234569999" className="text-white hover:text-[#D4AF37] transition-colors">
                  +91 9234569999
                </a>
              </div>
              <div className="flex items-center gap-3">
                <svg className="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-1.548.773a11.037 11.037 0 006.105 6.105l.774-1.548a1 1 0 011.059-.54l4.435.74a1 1 0 01.836.986V17a1 1 0 01-1 1h-2C7.82 18 2 12.18 2 5V3z" />
                </svg>
                <a href="tel:+919063960993" className="text-white hover:text-[#D4AF37] transition-colors">
                  +91 9063960993
                </a>
              </div>

              {/* Email */}
              <div className="flex items-center gap-3">
                <svg className="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 12a4 4 0 10-8 0 4 4 0 008 0zm0 0v1.5a2.5 2.5 0 005 0V12a9 9 0 10-9 9m4.5-1.206a8.959 8.959 0 01-4.5 1.207" />
                </svg>
                <a href="mailto:hello@sashaclinics.com" className="text-white hover:text-[#D4AF37] transition-colors">
                  hello@sashaclinics.com
                </a>
              </div>

              {/* Social Media Icons */}
              <div className="flex items-center gap-4 pt-2">
                <a href="#" className="text-white hover:text-[#D4AF37] transition-colors">
                  <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                  </svg>
                </a>
                <a href="#" className="text-white hover:text-[#D4AF37] transition-colors">
                  <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                  </svg>
                </a>
                <a href="#" className="text-white hover:text-[#D4AF37] transition-colors">
                  <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
                  </svg>
                </a>
              </div>
            </div>
          </motion.div>
        </div>
      </div>

      {/* Copyright Bar */}
      <motion.div 
        className="bg-[#3D1F58] py-4"
        initial={{ opacity: 0 }}
        whileInView={{ opacity: 1 }}
        viewport={{ once: true }}
        transition={{ duration: 0.5 }}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-white text-left text-sm">
            Copyright 2022 Sasha clinics. All rights reserved.
          </p>
        </div>
      </motion.div>
    </motion.footer>
  )
}

export default Footer

